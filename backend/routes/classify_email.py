...
async def classify_email(email_content: str):
    if not email_content or not email_content.strip():
        raise HTTPException(
            status_code=422,
            detail="O conteúdo do email não pode ser vazio"
        )
    ...
    if "Erro" in response["classification"] or "Indefinido" in response["classification"]:
        raise HTTPException(status_code=500, detail=f"Não foi possível classificar o email fornecido")
...
async def classify_from_file(email_file: Annotated[UploadFile, File()]):
    ...
    if email_file.filename.lower().endswith(".pdf"):
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Erro ao processar PDF")
    
    elif email_file.filename.lower().endswith(".txt"):
        try:
            email_content = extract_text_from_txt_bytes(file_bytes)
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Erro ao processar TXT")
    else:
        raise HTTPException(
            status_code=400,
            detail="Extensão de arquivo inválida. Os arquivos devem ser PDF ou TXT."
        )
